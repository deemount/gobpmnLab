...
func NewBpmnEngine(name string) BpmnEngine {
	...
	log.Printf("BpmnEngine.go: return new engine with name %s", name)
	return BpmnEngine{
		Name:             name,
		Processes:        []ProcessInfo{},
		ProcessInstances: []*ProcessInstanceInfo{},
...
func (engine *BpmnEngine) LoadFromFile(filename string) (*ProcessInfo, error) {
	var err error
	log.Printf("BpmnEngine.go: load bpmn file from %s", "files/bpmn/"+filename+".bpmn")
	xmlData, err := os.ReadFile("files/bpmn/" + filename + ".bpmn")
	if err != nil {
		return nil, err
	}
