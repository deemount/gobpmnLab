...
/*
	...
	Let's try, to figure out, which "build orders" are given

	1. Build Order:
		a) A Element has one Flow in his method
...
func (p DelegateParameter) defaultCoordinates() (int, int) {
	...
	log.Printf("models/bpmn/canvas/Delegates.go (defaultCoordinates): set default coordinates x: %d, y: %d for %s", x, y, p.T)
	return x, y
}
...
func (p *DelegateParameter) findCoordinatesByPreviousWaypoint() (int, int) {
	...
	log.Println("models/bpmn/canvas/Delegates.go (findCoordinatesByPreviousWaypoint): no bounds set in shape, but found previous waypoint")
	...
	log.Printf("models/bpmn/canvas/Delegates.go (findCoordinatesByPreviousWaypoint): got shape type %s", p.T)
	_, height := p.defaultElementSize()

	log.Printf("models/bpmn/canvas/Delegates.go (findCoordinatesByPreviousWaypoint): got previous waypoint %d, %d", p.WPPREV.X, p.WPPREV.Y)
	x := p.WPPREV.X
...
func (p *DelegateParameter) findCoordinatesByPreviousBounds() (int, int) {
	...
	log.Println("models/bpmn/canvas/Delegates.go (findCoordinatesByPreviousBounds): no bounds set in shape, but found previous bounds")
	...
	log.Printf("models/bpmn/canvas/Delegates.go (findCoordinatesByPreviousBounds): got shape type %s", p.T)
	_, height := p.defaultElementSize()

	log.Printf("models/bpmn/canvas/Delegates.go (findCoordinatesByPreviousBounds): got previous bounds %d, %d", p.BSPTR.X, p.BSPTR.Y)
...
func (p *DelegateParameter) setBounds() {
	...
	log.Printf("models/bpmn/canvas/Delegates.go (setBounds): set bounds (coordinates: %d, %d, size: %d, %d) for %s (%s)", p.B.X, p.B.Y, p.B.Width, p.B.Height, p.S.ID, p.S.Element)
...
func (p *DelegateParameter) setWaypoints() {
	...
	log.Printf("models/bpmn/canvas/Delegates.go (setWaypoints): got waypoints %d", len(p.W))

	if len(p.W) > 2 {
		// set many waypoints by calculating the length of the slice
		p.E.SetWaypoints(len(p.W))
	} else {
		// set two waypoints by default
		p.E.SetWaypoint()
	}
	...
	if len(p.W) == 0 {
		...
		log.Println("models/bpmn/canvas/Delegates.go (setWaypoints): no waypoints set in edge")

		// calculate the first waypoint X
		// e.g.
		// taking default values, which assigned above in defaultElementSize()
		// startevent X1: 179 (default)
		// edge (from StartEvent) X2: unknown (where the edge should start)
		// startevent (Width) X3: 36 (default)
		// I want to calculate X2 and Y2 (where the edge should start)
		...
		// set waypoints by given values in delegate parameter
		log.Printf("models/bpmn/canvas/Delegates.go (setWaypoints): set waypoints by given values in delegate parameter")
	...
	log.Printf("models/bpmn/canvas/Delegates.go (setWaypoints): set waypoints for %s", p.E.ID)
...
func SetShape(p DelegateParameter) {
	...
	log.Printf("models/bpmn/canvas/Delegates.go (setShape): set shape for %s (%s)", p.S.ID, p.S.Element)
...
func SetEdge(p DelegateParameter) {
	...
	p.setWaypoints()
	log.Printf("models/bpmn/canvas/Delegates.go (setEdge): set edge for %s (%s)", p.E.ID, p.E.Element)
}
...
func SetLabel(p DelegateParameter) {
	...
	log.Println("models/bpmn/canvas/Delegates.go (setLabel): set label")
}
...
// an end event (terminate and so on) at his X,Y-points and a default distance right
// to the element.
// calculating X and Y for a start event inside the first pool:
// e.g.
...
func SetPool(p DelegateParameter) {
	...
	log.Printf("models/bpmn/canvas/Delegates.go (setPool): set bounds for %s (%s)", p.S.ID, p.S.Element)
